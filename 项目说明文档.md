...
**本项目目前处于本地开发测试阶段**，已完成核心功能开发和管理后台美化。项目支持本地SQLite数据库运行，同时预留了MySQL数据库接口，便于后续生产环境部署。

#### 开发环境特点
- ✅ 本地SQLite数据库，无需额外配置
- ✅ 完整的管理后台界面，支持用户管理和内容管理
- ✅ 响应式设计，支持桌面端和移动端访问
- ✅ 示例数据预置，便于功能测试和演示
- ✅ 现代化UI设计，包含动画效果和交互优化
...
#### 3. 管理后台访问
- **管理员登录页面**: http://127.0.0.1:5000/admin/login
- **默认管理员账号**: admin / admin123
- **用户管理**: http://127.0.0.1:5000/admin/user_management
- **内容管理**: http://127.0.0.1:5000/admin/content_management
...
##### 创建Nginx配置文件 `/etc/nginx/sites-available/kunqing-campus`:
```nginx
server {
    listen 80;
    server_name your-domain.com;
...
##### 创建systemd服务文件 `/etc/systemd/system/kunqing-campus.service`:
```ini
[Unit]
...
##### 2. 转换数据格式
```python
# 创建转换脚本 convert_sqlite_to_mysql.py
import re
...
- 管理员登录入口：`/admin/login`。
- 权限装饰器：
  - `admin_required`：普通管理员权限校验。
  - `super_admin_required`：超级管理员权限校验（用于系统设置页）。
  - `user_management_required`、`content_management_required`：分别用于用户与内容管理。
- 设置页路由：`/admin/settings` 使用 `super_admin_required` 保护，仅 `admin_level>=2` 或学号为 `admin` 的账户可访问。非超管访问将返回 `403 Forbidden`。
- 账户提示：数据库的管理员初始密码为 `123456`（旧版SHA256哈希）。首次登录会自动迁移到 bcrypt 存储。

## ✉️ 邮件 SMTP 配置与测试

为便于部署前验证邮件能力，系统设置页提供 SMTP 自检与测试邮件发送功能，并在页面内联展示结果（包含耗时与错误信息）。

### 环境变量与配置项
- `MAIL_SERVER`：SMTP服务器地址（如 `smtp.qq.com`）
- `MAIL_PORT`：SMTP端口（如 `587`）
- `MAIL_USE_TLS`：是否启用TLS（推荐）
- `MAIL_USE_SSL`：是否启用SSL（通常与TLS互斥）
- `MAIL_USERNAME`：SMTP登录用户名（发件邮箱）
- `MAIL_PASSWORD`：SMTP登录密码或授权码（QQ邮箱需授权码）
- `MAIL_DEFAULT_SENDER`：默认发件人邮箱

### 前端使用说明
- 在“管理员后台 → 系统设置”页面中：
  - 填写“测试收件人”（必填）。
  - 点击“测试SMTP配置”或“测试邮件”按钮。
  - 页面下方结果面板会展示：
    - `ok`（是否成功）
    - `server`、`port`、`use_tls`、`use_ssl`、`sender`、`recipient`
    - `duration_ms`（耗时，毫秒）、`error`（错误信息/代码）

### 后端接口说明
- 路由：`POST /admin/settings/test-smtp`（`super_admin_required`）
- 请求：`FormData` 或 `application/json`，字段 `email`（测试收件人）
- 响应示例：
```
{
  "ok": true,
